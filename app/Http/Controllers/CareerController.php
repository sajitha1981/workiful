    ...
    public function index(Request $request)
    {
        if (session('sesAdminID') == null ) {
			...
			try {
				...
				if ($request->ajax()) { 
					try {
						...
						if ($tp != null && $delids != null) {
							$result = $get->delCareer($delids, $tp);
						}
						$eduList = $get->getCareerDetails($npage, $orderby);
						$eduCnt = $get->getCareerCount();
						if ($endcnt > $eduCnt)
						{
							$endcnt = $eduCnt;
						}
					...
					return view('Master-Admin.careerload', ['eduList' => $eduList,
				...
				return view($this->AdminURL.'.manage-careers')->with($data);
			} catch (\Exception $e) {
