    ...
    public function index(Request $request)
    {
        if (session('sesAdminID') == null ) {
			...
			try {
				...
				if ($request->ajax()) { 
					try {
						...
						if ($tp != null && $delids != null) {
							$result = $get->delCity($delids, $tp);
						}
						$eduList = $get->getCityDetails($npage, $orderby);
						$eduCnt = $get->getCityCount();
						if ($endcnt > $eduCnt)
						{
							$endcnt = $eduCnt;
						}
					...
					return view('Master-Admin.cityload', ['eduList' => $eduList,
				...
				return view($this->AdminURL.'.manage-cities')->with($data);
			} catch (\Exception $e) {
