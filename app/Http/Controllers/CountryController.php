    ...
    public function index(Request $request)
    {
        if (session('sesAdminID') == null ) {
			...
			try {
				...
				if ($request->ajax()) { 
					try {
						...
						if ($tp != null && $delids != null) {
							$result = $get->delCountry($delids, $tp);
						}
						$eduList = $get->getCountryDetails($npage, $orderby);
						$eduCnt = $get->getCountryCount();
						if ($endcnt > $eduCnt)
						{
							$endcnt = $eduCnt;
						}
					...
					return view('Master-Admin.countryload', ['eduList' => $eduList,
				...
				return view($this->AdminURL.'.manage-countries')->with($data);
			} catch (\Exception $e) {
