    ...
    public function index(Request $request)
    {
        if (session('sesAdminID') == null ) {
			...
			try {
				...
				if ($request->ajax()) { 
					try {
						...
						if ($tp != null && $delids != null) {
							$result = $get->delIndustry($delids, $tp);
						}
						$eduList = $get->getIndustryDetails($npage, $orderby);
						$eduCnt = $get->getIndustryCount();
						if ($endcnt > $eduCnt)
						{
							$endcnt = $eduCnt;
						}
					...
					return view('Master-Admin.industryload', ['eduList' => $eduList,
				...
				return view($this->AdminURL.'.manage-industries')->with($data);
			} catch (\Exception $e) {
