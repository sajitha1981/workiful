...
class JobcategoryController extends Controller {
	...
	public function __construct()
	{
		...
		$this->curPage = "list-jobcategories";
    ...
    public function index(Request $request)
    {
        if (session('sesAdminID') == null ) {
			...
			try {
				...
				if ($request->ajax()) { 
					try {
						...
						if ($tp != null && $delids != null) {
							$result = $get->delJobcategory($delids, $tp);
						}
						$eduList = $get->getJobcategoryDetails($npage, $orderby);
						$eduCnt = $get->getJobcategoryCount();
						if ($endcnt > $eduCnt)
						{
							$endcnt = $eduCnt;
						}
					...
					return view('Master-Admin.jobcategoryload', ['eduList' => $eduList,
				...
				return view($this->AdminURL.'.manage-jobcategories')->with($data);
			} catch (\Exception $e) {
