    ...
    public function index(Request $request)
    {
        if (session('sesAdminID') == null ) {
			...
			try {
				...
				if ($request->ajax()) { 
					try {
						...
						if ($tp != null && $delids != null) {
							$result = $get->delState($delids, $tp);
						}
						$eduList = $get->getStateDetails($npage, $orderby);
						$eduCnt = $get->getStateCount();
						if ($endcnt > $eduCnt)
						{
							$endcnt = $eduCnt;
						}
					...
					return view('Master-Admin.stateload', ['eduList' => $eduList,
				...
				return view($this->AdminURL.'.manage-states')->with($data);
			} catch (\Exception $e) {
