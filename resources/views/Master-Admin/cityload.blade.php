            ...
            <thead class="cf">
              <tr>
                ...
                <th class="numeric" style="background:#03314b;color:#ffffff">No.</th>
				 <th style="background:#03314b;color:#ffffff">Country</th>
                <th style="background:#03314b;color:#ffffff">State</th>
				 <th style="background:#03314b;color:#ffffff">City</th>
...
<?php $index = $stcnt; ?>
			@if (count($eduList) > 0)
              ...
              <tr>
                <!-- All actions td start -->
                <td data-title="All"><input name="del[]" type="checkbox" value="{{ $val->city_id }}"  /></td>
                <!-- /All actions td -->
                <td data-title="No">{{ $index }}</td>
				 <td data-title="Country"> {{ $val->country_name }}</td>
                <td data-title="State"> {{ $val->state_name }}</td>
				 <td data-title="City"> {{ $val->city_name }}</td>
                <td data-title="Status">
					@if ($val->city_status == 1)
						Active
					@else 
						Inactive
					@endif
                ...
                <td data-title="Actions">
                  ...
                  <div class="btn-group dropup">
                    ...
                    <ul class="dropdown-menu pull-right" role="menu">
                      ...
                      <li><a href="#" class="delete-record" data-value="{{ $val->city_id }}"><i class="fa fa-trash"></i> Delete Record</a></li>
                    </ul>
                  </div>
                  <!-- /Actions html -->
                </td>
              ...
              <!-- /Data Row -->
               @endforeach
			@else
				<tr align='center' valign='middle'><td class='' colspan='5'>No city found.</td></tr>   
			@endif
</div>
 </tbody>
          </table>
